    ...
    for (auto&& node_ptr : model->get_ordered_ops()) {
        if (ov::op::util::is_constant(node_ptr)) {
            ...
            if (!inserted.second && inserted.first->second != offset) {
                LOG_WARN("Model contains constants with the same data pointer and different source offsets. "
                         "Disabling weightless serialization for this model.");
                m_const_to_offset.clear();
                return;
            }
        }
    }
